...
final class IdentityDiagnostics {
    enum Event { START, GET, POST, OTHER_METHOD, COMMIT, FINISH, UPGRADE, LOOP, ERROR, HTTP_ERROR, SSL_ERROR, DOCUMENT, RETRY, SCRIPT_ERROR, BLOCKED_MAIN, BLOCKED_FRAME, LENIENT_URL }
    ...
    static String route(String value){
        ...
        // Report the payment host for debugging without permitting navigation elsewhere.
        if(!"authserver.jhun.edu.cn".equals(host)&&!"ehall.jhun.edu.cn".equals(host)&&!"hqfw.jhun.edu.cn".equals(host)&&!"h5cloud.17wanxiao.com".equals(host))return "[其他或错误页面]";
        String path=u.getPath();if(path==null)path="";path=path.replaceAll(";[^/]*","");
        if(!path.matches("/(?:authserver/login|new/index\\.html|login|appShow|wsbx/?|wsbx/login/cas|wsbx/html/yd/wsbx\\.html|wsbx/html/pc/index\\.html|CloudPayment/user/getRoomState\\.do)"))path="/[其他路径]";
