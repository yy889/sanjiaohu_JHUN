...
final class UiSheet {
    ...
    UiSheet(SheetHost a,String title,String subtitle,float fraction){
        Activity host=(Activity)a;
        dialog=new Dialog(host);dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        FrameLayout outside=new FrameLayout(host);outside.setOnClickListener(v->dialog.dismiss());
        ...
        LinearLayout panel=new LinearLayout(host){@Override protected void onMeasure(int w,int h){int available=View.MeasureSpec.getSize(h);super.onMeasure(w,View.MeasureSpec.makeMeasureSpec((int)(available*fraction),View.MeasureSpec.EXACTLY));}};
        panel.setOrientation(LinearLayout.VERTICAL);panel.setBackground(a.shape(a.palette().sheetSurface,28));panel.setClipToOutline(true);panel.setOnClickListener(v->{});
        ...
        ScrollView scroll=new ScrollView(host);scroll.setVerticalScrollBarEnabled(false);scroll.setClipToPadding(false);body=a.column();body.setPadding(a.dp(22),a.dp(6),a.dp(22),a.dp(16));scroll.addView(body);panel.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));
    ...
    void show(SheetHost a){dialog.show();dialog.getWindow().setLayout(Math.min(a.screenWidth(),a.dp(560)),-1);}
    void actions(SheetHost a,String confirm,Runnable save){TextView cancel=a.themedButton("取消",()->dialog.dismiss(),false);footer.addView(cancel,new LinearLayout.LayoutParams(a.dp(76),a.dp(46)));TextView done=a.themedButton(confirm,save,true);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(0,a.dp(46),1);lp.leftMargin=a.dp(12);footer.addView(done,lp);}
}
